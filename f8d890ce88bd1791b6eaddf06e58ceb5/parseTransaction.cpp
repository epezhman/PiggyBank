...
class Transaction{
    ...
    public:
        Transaction (std::string, std::string, double, std::string);
...
Transaction::Transaction(std::string tSender, std::string tReceiver, double tAmount, std::string tToken){
...
void parseTransactionFile(Transaction* t, std::string fileName){
    ...
    try{
        ...
        int fieldCount = 1; // Expected to be only 4
        tFile.read(c, 1); // Read a character from the file
        while(c[0] != EOF && fieldCount < 5){  
            while(c[0] != '\n'){
                currentField += c[0];
                tFile.read(c, 1);
            }
...
int main(int argc, char* argv[]){
    ...
    try{
        ...
        std::cout << currentTransaction.getSender() << ":" << currentTransaction.getReceiver() << ":" << currentTransaction.getAmount() << ":" << currentTransaction.getToken() << ";";
        return 1;
   }catch(std::exception& e){
       std::cout << "Error encountered: " << e.what() << std::endl;
       return -1;
